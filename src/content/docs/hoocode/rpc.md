...
**Extension commands**: If the message is an extension command (e.g., `/mycommand`), it executes immediately even during streaming. Extension commands manage their own LLM interaction via `hoo.sendMessage()`.
...
- `source`: What kind of command:
  - `"extension"`: Registered via `hoo.registerCommand()` in an extension
