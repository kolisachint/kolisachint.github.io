...
Extensions can register custom model providers via `hoo.registerProvider()`. This enables:
...
export default function (hoo: ExtensionAPI) {
  ...
  hoo.registerProvider("my-provider", {
    name: "My Provider",
...
Use `hoo.unregisterProvider(name)` to remove a provider that was previously registered via `hoo.registerProvider(name, ...)`:
...
```typescript
hoo.registerProvider("custom-api", {
  baseUrl: "https://api.example.com",
  apiKey: "MY_API_KEY",
