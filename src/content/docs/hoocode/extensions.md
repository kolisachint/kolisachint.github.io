...
**Key capabilities:**
- **Custom tools** - Register tools the LLM can call via `hoo.registerTool()`
- **Event interception** - Block or modify tool calls, inject context, customize compaction
- **User interaction** - Prompt users via `ctx.ui` (select, confirm, input, notify)
- **Custom UI components** - Full TUI components with keyboard input via `ctx.ui.custom()` for complex interactions
- **Custom commands** - Register commands like `/mycommand` via `hoo.registerCommand()`
- **Session persistence** - Store state that survives restarts via `hoo.appendEntry()`
...
export default function (hoo: ExtensionAPI) {
  // Subscribe to events
  hoo.on("event_name", async (event, ctx) => {
...
If the factory returns a `Promise`, hoocode awaits it before continuing startup. That means async initialization completes before `session_start`, before `resources_discover`, and before provider registrations queued via `hoo.registerProvider()` are flushed.
...
export default async function (hoo: ExtensionAPI) {
  ...
  hoo.registerProvider("local-openai", {
...
thinking level changes (settings, keybinding, hoo.setThinkingLevel())
...
```typescript
hoo.on("resources_discover", async (event, _ctx) => {
...
hoo.on("session_compact", async (event, ctx) => {
...
```typescript
hoo.on("session_before_tree", async (event, ctx) => {
...
hoo.on("session_tree", async (event, ctx) => {
...
```typescript
hoo.on("session_shutdown", async (event, ctx) => {
...
```typescript
hoo.on("before_agent_start", async (event, ctx) => {
...
```typescript
hoo.on("turn_start", async (event, ctx) => {
...
```typescript
hoo.on("message_start", async (event, ctx) => {
  // event.message
});

hoo.on("message_update", async (event, ctx) => {
  // event.message
  // event.assistantMessageEvent (token-by-token stream event)
});

hoo.on("message_end", async (event, ctx) => {
...
```typescript
hoo.on("tool_execution_start", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args
});

hoo.on("tool_execution_update", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args, event.partialResult
});

hoo.on("tool_execution_end", async (event, ctx) => {
...
```typescript
hoo.on("context", async (event, ctx) => {
...
```typescript
hoo.on("before_provider_request", (event, ctx) => {
...
```typescript
hoo.on("after_provider_response", (event, ctx) => {
...
```typescript
hoo.on("thinking_level_select", async (event, ctx) => {
...
Use this to update extension UI when `hoo.setThinkingLevel()`, model changes, or built-in thinking-level controls change the active thinking level.
...
hoo.on("tool_result", async (event, ctx) => {
...
```typescript
hoo.on("before_agent_start", (event, ctx) => {
...
- `withSession`: run post-switch work against a fresh replacement-session context. Do not use captured old `hoo` / command `ctx`; see [Session replacement lifecycle and footguns](#session-replacement-lifecycle-and-footguns).
...
- Captured old `hoo` / old command `ctx` session-bound objects are stale after replacement and will throw if used. Use only the `ctx` passed to `withSession` for session-bound work.
...
`hoo.registerTool()` works both during extension load and after startup. You can call it inside `session_start`, command handlers, or other event handlers. New tools are refreshed immediately in the same session, so they appear in `hoo.getAllTools()` and are callable by the LLM without `/reload`.

Use `hoo.setActiveTools()` to enable or disable tools (including dynamically added tools) at runtime.
...
### hoo.sendMessage(message, options?)
...
### hoo.sendUserMessage(content, options?)

Send a user message to the agent. Unlike `sendMessage()` which sends custom messages, this sends an actual user message that appears as if typed by the user. Always triggers a turn.

```typescript
// Simple text message
hoo.sendUserMessage("What is 2+2?");
...
hoo.sendUserMessage("Focus on error handling", { deliverAs: "steer" });
hoo.sendUserMessage("And then summarize", { deliverAs: "followUp" });
```
...
### hoo.appendEntry(customType, data?)

Persist extension state (does NOT participate in LLM context).

```typescript
hoo.appendEntry("my-state", { count: 42 });
...
```typescript
hoo.setSessionName("Refactor auth module");
```
...
```typescript
const name = hoo.getSessionName();
if (name) {
  console.log(`Session: ${name}`);
}
```

### hoo.setLabel(entryId, label)

Set or clear a label on an entry. Labels are user-defined markers for bookmarking and navigation (shown in `/tree` selector).

```typescript
// Set a label
hoo.setLabel(entryId, "checkpoint-before-refactor");

// Clear a label
hoo.setLabel(entryId, undefined);
...
### hoo.registerCommand(name, options)
...
### hoo.registerMessageRenderer(customType, renderer)

Register a custom TUI renderer for messages with your `customType`. See [Custom UI](#custom-ui).

### hoo.registerShortcut(shortcut, options)

Register a keyboard shortcut. See [keybindings.md](keybindings.md) for the shortcut format and built-in keybindings.

```typescript
hoo.registerShortcut("ctrl+shift+p", {
...
### hoo.exec(command, args, options?)

Execute a shell command.

```typescript
const result = await hoo.exec("git", ["status"], { signal, timeout: 5000 });
// result.stdout, result.stderr, result.code, result.killed
```

### hoo.getActiveTools() / hoo.getAllTools() / hoo.setActiveTools(names)

Manage active tools. This works for both built-in tools and dynamically registered tools.

```typescript
const active = hoo.getActiveTools();
const all = hoo.getAllTools();
// [{
//   name: "read",
...
hoo.setActiveTools(["read", "bash"]); // Switch to read-only
```

`hoo.getAllTools()` returns `name`, `description`, `parameters`, and `sourceInfo`.
...
### hoo.getThinkingLevel() / hoo.setThinkingLevel(level)
...
```typescript
const current = hoo.getThinkingLevel();  // "off" | "minimal" | "low" | "medium" | "high" | "xhigh"
hoo.setThinkingLevel("high");
```
...
```typescript
hoo.events.on("my:event", (data) => { ... });
hoo.events.emit("my:event", { ... });
```

### hoo.registerProvider(name, config)
...
```typescript
hoo.registerCommand("my-setup-teardown", {
...
Register tools the LLM can call via `hoo.registerTool()`. Tools appear in the system prompt and can have custom rendering.

Use `promptSnippet` for a short one-line entry in the `Available tools` section in the default system prompt. If omitted, custom tools are left out of that section.

Use `promptGuidelines` to add tool-specific bullets to the default system prompt `Guidelines` section. These bullets are included only while the tool is active (for example, after `hoo.setActiveTools([...])`).
...
hoo.registerTool({
  ...
  async execute(toolCallId, params, signal, onUpdate, ctx) {
    ...
    // Run commands via hoo.exec (captured from extension closure)
    const result = await hoo.exec("some-command", [], { signal });
...
export default function (hoo: ExtensionAPI) {
  ...
  hoo.registerTool({ name: "db_connect", ... });
  hoo.registerTool({ name: "db_query", ... });
  hoo.registerTool({ name: "db_close", ... });
...
hoo.registerMessageRenderer("my-extension", (message, options, theme) => {
...
Messages are sent via `hoo.sendMessage()`:
...
| `event-bus.ts` | Inter-extension events | `hoo.events` |
| **Session Metadata** |||
